    ...
    if st.button("🚀 Process Presentation Deck", type="primary"):
        if not api_key_input:
            ...
            try:
                ...
                for idx in range(1, total_slides + 1):
                    audio_out = os.path.join(audio_dir, f"slide_{idx}.mp3")
                    script_text = slide_scripts[idx - 1]
                    audio_generator.generate_slide_audio(script_text, audio_out, voice_profile=selected_voice)
